...
import { stages, StageProgressionManager, Floor, MonsterSet } from "../data/stages";
...
export class StageManager {
   ...
   */
  static createStage(stageId: string, floorIndex: number): {
    enemies: EnemyModel[];
    message: MessageType;
    scale?: number;
    bg?: string;
    ...
    if (floor.isBossFloor) {
      ...
      // ランダムに選択
      selectedMonsterSet = StageProgressionManager.selectRandomMonsterSet(floor);
    }
    ...
    const enemies = selectedMonsterSet.monsters.map((enemyData, index) => {
      ...
      enemyInstance.positionOffset = selectedMonsterSet!.positions[index] || { x: 0, y: 0 };
   ...
   * @returns 生成したステージ情報
   */
  static createNewStage(stageId?: string, floorIndex?: number): {
    enemies: EnemyModel[];
    message: MessageType;
    scale?: number;
    isBossFloor?: boolean;
  ...
  private static createDefaultStage(): {
    ...
    const enemies = defaultMonsterSet.monsters.map((enemyData, index) => {
      const enemyInstance = new EnemyModel(enemyData);
      enemyInstance.positionOffset = defaultMonsterSet.positions[index] || { x: 0, y: 0 };
      return enemyInstance;
    });
 ...
 * @returns メッセージオブジェクト
 */
static createCompletionMessage(totalExp: number, isBossFloor?: boolean): MessageType {
  if (isBossFloor) {
    return {
