...
interface SkillSlotProps {
    ...
    index: number;
    isActive: boolean;
    isKeyPressed: boolean; // ファンクションキーが押されたかどうか
    cooldown: number;
...
const SkillSlot: React.FC<SkillSlotProps> = ({
    ...
    if (!skill) {
        return (
            <div
                className="w-8 h-8 bg-gray-700 rounded-md border border-gray-500 flex items-center justify-center cursor-pointer text-xs skill-slot"
                onClick={handleClick}
            >
    ...
    // キー押下時の追加スタイル
    const keyPressStyle = isKeyPressed
        ? 'ring-2 ring-yellow-400 transform scale-110 transition-transform duration-100'
        : '';
